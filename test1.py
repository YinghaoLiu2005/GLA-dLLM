...
def main():
    ...
    try:
        base_model = AutoModel.from_pretrained(fast_dllm_model_path,local_files_only=True, trust_remote_code=True)
    ...
    # 下面会执行 from_fast_dllm_pretrained 中的复制逻辑并打印每个键的初始化状态
    gla_from_fast = GLADreamModel.from_fast_dllm_pretrained(fast_dllm_model_path,cfg,local_files_only=True,trust_remote_code=True )
    # 可选：把最终 state_dict 写出便于检查
    torch.save(gla_from_fast.state_dict(), "gla_initialized_from_fast_dllm.pt")
    print("Finished from_fast_dllm_pretrained")
