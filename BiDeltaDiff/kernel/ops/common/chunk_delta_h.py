...
@triton.jit(do_not_specialize=['T', 'num_clean_chunks'])
def chunk_gated_delta_rule_fwd_kernel_h_blockdim64(
    ...
    if USE_CLEAN_STATES:
        clean_states_base=clean_states + (i_nh * num_clean_chunks*K*V).to(tl.int64)

    # main recurrence
    for i_t in range(NT):

        if USE_CLEAN_STATES:
            # 如果进入 Noisy 阶段 (i_t >= num_clean_chunks)
            if i_t >= num_clean_chunks:
                # 计算需要读取哪个 Clean Block 的状态 (Block N 读取 Block N-1)
                target_idx = i_t - num_clean_chunks - 1
                
                if target_idx < 0:
                    # Noisy Block 0: 初始状态置 0 (阻断与 Clean 的直接连接，或视为开头)
                    b_h1 = tl.zeros([64, BV], dtype=tl.float32)
                    ...
                    # 从 Buffer 中读取 Clean State
                    # 注意：这里实现了 "Noisy N" 读取 "Clean N-1"
                    curr_clean_ptr = clean_states_base + (target_idx * K * V).to(tl.int64)
                    
                    p_cs_1 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (0, i_v * BV), (64, BV), (1, 0))
                    b_h1 = tl.load(p_cs_1, boundary_check=(0, 1)).to(tl.float32)
                    if K > 64:
                        p_cs_2 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0))
                        b_h2 = tl.load(p_cs_2, boundary_check=(0, 1)).to(tl.float32)
                    if K > 128:
                        p_cs_3 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0))
                        b_h3 = tl.load(p_cs_3, boundary_check=(0, 1)).to(tl.float32)
                    if K > 192:
                        p_cs_4 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0))
                        b_h4 = tl.load(p_cs_4, boundary_check=(0, 1)).to(tl.float32)
        ...
        if USE_CLEAN_STATES:
            # 只有 Clean Block 允许写入状态 Buffer
            if i_t < num_clean_chunks:
                curr_clean_ptr = clean_states_base + (i_t * K * V).to(tl.int64)
                
                # 将 *更新后* 的 b_h1 (Output State of current block) 写入 Buffer
                p_cs_1 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (0, i_v * BV), (64, BV), (1, 0))
                tl.store(p_cs_1, b_h1.to(p_cs_1.dtype.element_ty), boundary_check=(0, 1))
                if K > 64:
                    p_cs_2 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0))
                    tl.store(p_cs_2, b_h2.to(p_cs_2.dtype.element_ty), boundary_check=(0, 1))
                if K > 128:
                    p_cs_3 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0))
                    tl.store(p_cs_3, b_h3.to(p_cs_3.dtype.element_ty), boundary_check=(0, 1))
                if K > 192:
                    p_cs_4 = tl.make_block_ptr(curr_clean_ptr, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0))
                    tl.store(p_cs_4, b_h4.to(p_cs_4.dtype.element_ty), boundary_check=(0, 1))        
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    if USE_CLEAN_STATES:
        d_clean_states_base = d_clean_states + (i_nh * num_clean_chunks * K * V).to(tl.int64)
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if USE_CLEAN_STATES:
            # 逻辑：
            # 1. 如果当前是 Noisy Block (i_t >= num_clean_chunks):
            #    当前累积的 b_dh 是 Noisy Block *开头* 的梯度。
            #    这个梯度必须传给对应的 Clean Block *结尾* (d_clean_states)。
            #    然后 b_dh 必须重置 (因为 Noisy Block 之间不传梯度)。
            
            if i_t >= num_clean_chunks:
                target_idx = i_t - num_clean_chunks - 1
                
                if target_idx >= 0:
                    # 将 b_dh 存入 d_clean_states
                    curr_d_clean_ptr = d_clean_states_base + (target_idx * K * V).to(tl.int64)
                    
                    p_dcs_1 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (0, i_v * BV), (64, BV), (1, 0))
                    tl.store(p_dcs_1, b_dh1.to(p_dcs_1.dtype.element_ty), boundary_check=(0, 1))
                    if K > 64:
                        p_dcs_2 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0))
                        tl.store(p_dcs_2, b_dh2.to(p_dcs_2.dtype.element_ty), boundary_check=(0, 1))
                    if K > 128:
                        p_dcs_3 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0))
                        tl.store(p_dcs_3, b_dh3.to(p_dcs_3.dtype.element_ty), boundary_check=(0, 1))
                    if K > 192:
                        p_dcs_4 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0))
                        tl.store(p_dcs_4, b_dh4.to(p_dcs_4.dtype.element_ty), boundary_check=(0, 1))
                
                # 重置 b_dh (阻断梯度流向 Noisy i-1)
                b_dh1 = tl.zeros([64, BV], dtype=tl.float32)
                ...
                # 如果当前是 Clean Block (i_t < num_clean_chunks):
                # 刚算完 Block i_t，现在需要处理它 *结尾* 的梯度。
                # 梯度来源：下一个 Clean Block 的开头 + (关键) Noisy Block 读取它产生的梯度。
                
                # 从 d_clean_states 读取梯度并累加
                curr_d_clean_ptr = d_clean_states_base + (i_t * K * V).to(tl.int64)
                
                p_dcs_1 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (0, i_v * BV), (64, BV), (1, 0))
                b_dh1 += tl.load(p_dcs_1, boundary_check=(0, 1))
                if K > 64:
                    p_dcs_2 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (64, i_v * BV), (64, BV), (1, 0))
                    b_dh2 += tl.load(p_dcs_2, boundary_check=(0, 1))
                if K > 128:
                    p_dcs_3 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (128, i_v * BV), (64, BV), (1, 0))
                    b_dh3 += tl.load(p_dcs_3, boundary_check=(0, 1))
                if K > 192:
                    p_dcs_4 = tl.make_block_ptr(curr_d_clean_ptr, (K, V), (V, 1), (192, i_v * BV), (64, BV), (1, 0))
                    b_dh4 += tl.load(p_dcs_4, boundary_check=(0, 1))
