...
class GatedDeltaNet(nn.Module):
    ...
    def apply_diffusion_concat_init(self):
        """
        initialize the output projection weights to [0.5*I, 0.5*I] for cold start
        """
        with torch.no_grad():
            # weight 形状: [out_features, in_features] -> [hidden_size, 2 * value_dim]
            weight = self.o_proj.weight
            out_dim, in_dim = weight.shape
            v_dim = in_dim // 2  # 单路 value 维度
            ...
            # 2. 构造两个 0.5 * I 的对角模式
            # 遍历输出维度和单路输入维度的最小值
            # 即使 v_dim > hidden_size (常见情况)，这种初始化也能确保前 D 个通道的 identity 映射
            fill_range = min(out_dim, v_dim)
            for i in range(fill_range):
                # 前半部分权重 (正向)
                weight[i, i] = 0.5
                # 后半部分权重 (反向)
                weight[i, i + v_dim] = 0.5
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        L=num_clean_chunks*bd_size
        o_fwd,o_bwd=None,None

        if mode == 'chunk':
            o_fwd, recurrent_state = chunk_gated_delta_rule(
                q=q,
                k=k,
                v=v,
                g=g,
            ...
            if self.is_bidirectional and q_len > L:
                ...
                # 执行反向扫描 (num_clean_chunks 设为 0，因为块间不传递)
                o_bwd, _ = chunk_gated_delta_rule(
                    q=q_rev,
                    k=k_rev,
                    v=v_rev,
                    g=g_rev,
                    beta=b_rev,
                    initial_state=None, # 反向流是块独立的，不需要初始状态
                    output_final_state=False,
                    cu_seqlens=None, # 此时已经 fold 到 batch，不需要变长处理
                    use_qk_l2norm_in_kernel=True,
                    num_clean_chunks=0 
                )
                
                # 翻转回来并还原形状
                o_bwd_noisy = torch.flip(o_bwd, dims=[1])
                o_bwd_noisy = rearrange(o_bwd_noisy, '(b n) c h v -> b (n c) h v', b=batch_size)
        ...
        if self.use_gate:
            ...
            o_bwd = self.o_norm(o_bwd, g)
        else:
            o_fwd = self.o_norm(o_fwd)
